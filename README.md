...
Notice: Two rotations are applied to each endpiece. To avoid order-dependent artifacts from _*EULER ANGLE*_, we decouple the transformations by applying rotateZ in the local coordinate system and rotateX in the world coordinate system. This ensures the two rotations remain independent and removes ambiguity caused by rotation order
